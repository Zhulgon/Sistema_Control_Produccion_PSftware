...
def run_demo() -> dict:
    cnc = CNCFactory().create_machine()
    robot = RobotFactory().create_machine()
    planned_units = 500
    return {
        "cnc": {"machine": type(cnc).__name__, "start": cnc.start(), "produced": cnc.produce(planned_units)},
        "robot": {"machine": type(robot).__name__, "start": robot.start(), "produced": robot.produce(planned_units)},
    }
